...
```bash
pnpm dlx --package=@tiangong-lca/cli@0.1.20 tiangong-lca auth status --json
```
...
- wrappers default to the exact published CLI through `pnpm dlx --package=@tiangong-lca/cli@0.1.20 tiangong-lca`; sibling directories are never auto-discovered
...
- local CLI overrides must identify `@tiangong-lca/cli@0.1.20` with its exact Node/pnpm engines and a v9 `pnpm-lock.yaml` and the published TIDAS source manifest (spec 0.2.2); stale local builds are installed with `pnpm install --frozen-lockfile` before `pnpm run build`
