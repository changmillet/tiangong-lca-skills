...
lastReviewedNote: "Reviewed for Skills #105：活动 wrapper 已校验已发布 CLI 0.1.16 及其 source-bound TIDAS manifest；历史 Foundry 0.1.8 bootstrap 仍独立固定 CLI 0.1.14。复制安装验收要求覆盖四个平台。"
---
...
完整入口随包提供 [Foundry 0.1.8](https://github.com/tiangong-lca/data-foundry/releases/tag/foundry-runtime-v0.1.8) 的最终发行锁，历史上固定 CLI 0.1.14、Node 24.19.0 和 TIDAS 0.3.0。公开运行时已通过 macOS arm64、Linux x64/arm64 和 Windows x64 验证。共享 wrapper 与 hybrid-search 包当前固定已发布 CLI 0.1.16；历史 Foundry bootstrap 脚本/lock 与活动 wrapper 相互独立。安装或复制入口时，保持随包脚本与相邻 lock 完整。安装、登录不授予数据写入权限；继续执行任务当前的授权与恢复动作。
...
- 只校验本次变更的 skill:
  ...
  ```
- CI 会在 `.github/workflows/validate-skills.yml` 中 checkout 活动 CLI commit `4316c205453071c8cbb45e06480344f8eae5e041`（发布包 0.1.16），用 frozen pnpm lockfile 安装两个仓库并构建 CLI，然后运行同一套校验。
...
- skill wrapper 默认使用精确版本的已发布 CLI：`pnpm dlx --package=@tiangong-lca/cli@0.1.16 tiangong-lca`；不会自动发现任何 sibling 目录
- 本地执行只能通过 `--cli-dir` / `TIANGONG_LCA_CLI_DIR` 显式启用
- 使用 `--published-cli` 可覆盖本地 CLI 环境并显式执行 published-package case；嵌套 wrapper 会继续传播该选择
- 本地 CLI override 必须是带精确 Node/pnpm engines、v9 `pnpm-lock.yaml` 和已发布 TIDAS source manifest 的 `@tiangong-lca/cli@0.1.16`；本地 build 过期时先执行 `pnpm install --frozen-lockfile`，再执行 `pnpm run build`
