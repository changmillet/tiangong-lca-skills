...
lastReviewedNote: 'Reviewed for Skills #104 at head 862e104 (final guidance phase): the guidance now also holds the pinned-runtime gap as an explicit stop-and-report (pinned published CLI 0.1.18 and historical Foundry 0.1.8 predate the unknown-`[]` policy, the validation-layer report and the CLI #283 repair entry), forbids hand-fabricating the empty array or bypassing the runtime, and keeps merged source behavior separate from published support; four new negative doc guards, five required doc patterns and the extended semantic contract test enforce it. Published CLI 0.1.18 and the historical Foundry 0.1.8 lock stay byte-unchanged.'
related:
  - .docpact/config.yaml
...
- Node package execution is pinned to Node `24.19.0` and pnpm `11.24.0`; the default runtime is the exact published `@tiangong-lca/cli@0.1.18` and must never float through `@latest`.
